...
public:
	...
	void Save(){
		...
		FILEOUT << std::fixed << std::setprecision(8) << uv_kernel_size << " " << uv_kernel_stride << " " << uv_kernel_oversampling << " " << w_kernel_size << " " << w_kernel_stride << " " << w_kernel_oversampling << " " << grid_z << " " << grid_y << " " << grid_x << " " << nVisibilities << " " << nSubgrids << " " << nRuns << " " << degrid_time << " " << kernel << " " << vis_type << " " << test_type << endl;
		FILEOUT.close();
	}
	
	void Print(){
		cout << std::fixed << std::setprecision(8) << uv_kernel_size << " " << uv_kernel_stride << " " << uv_kernel_oversampling << " " << w_kernel_size << " " << w_kernel_stride << " " << w_kernel_oversampling << " " << grid_z << " " << grid_y << " " << grid_x << " " << nVisibilities << " " << nSubgrids << " " << nRuns << " " << degrid_time << " " << kernel << " " << vis_type << " " << test_type << endl;
	}
